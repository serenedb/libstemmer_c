...
static const unsigned short a_0[] = {
    0x0006 , 0xFCE1 , 0xFFFF , 0x0000 , 0x0000 , 0xFFFF , 0x0000 , 0x0000 ,
    0x0000 , 0x0000 , 0xFFFE , 0x0000 , 0xFFFE , 0x0000 , 0xFFFD , 0x0000 ,
    0xFFFD , 0x0000 , 0x0000 , 0x0000 , 0xFFFC , 0x0000 , 0x0000 , 0xFFFC ,
    0x0000 , 0x0000 , 0x0000 , 0xFFFB , 0x0000 , 0xFFFB
};
...
extern int dutch_porter_ISO_8859_1_stem(struct SN_env * z) {
    ...
    z->lb = z->c; z->c = z->l;
    {
        ...
        switch (among_var) {
            ...
            case 3:
                if (((SN_local *)z)->i_p1 > z->c) goto lab10;
                if (out_grouping_b(z, g_v_j, 97, 232, 0)) goto lab10;
                {
                    int ret = slice_del(z);
                    if (ret < 0) return ret;
                }
    ...
    {
        int v_16 = z->l - z->c;
        if (out_grouping_b(z, g_v_I, 73, 232, 0)) goto lab17;
        {
            int v_17 = z->l - z->c;
            if (z->c - 1 <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((2129954 >> (z->p[z->c - 1] & 0x1f)) & 1)) goto lab17;
            if (!find_among_b(z, a_5)) goto lab17;
            if (out_grouping_b(z, g_v, 97, 232, 0)) goto lab17;
            z->c = z->l - v_17;
        }
        z->ket = z->c;
        if (z->c <= z->lb) goto lab17;
        z->c--;
        z->bra = z->c;
        {
            int ret = slice_del(z);
