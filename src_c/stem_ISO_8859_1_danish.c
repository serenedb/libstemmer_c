...
#define s_0 (s_2 + 2)
static const symbol s_2[] = { 'l', 0xF8, 's', 't' };
...
static const unsigned short a_2[] = {
    ...
    0xFFFF , 0x0000 , 0x0002 , 0xFFFF , S(6C65), 0x0000 , 0x0003 , 0xFFFE ,
    S(F86C), S(0073)
};
...
extern int danish_ISO_8859_1_stem(struct SN_env * z) {
    ...
    z->lb = z->c; z->c = z->l;
    {
        ...
        switch (among_var) {
            case 1:
                ...
                break;
            case 2:
                if (in_grouping_b(z, g_s_ending, 39, 229, 0)) goto lab4;
                {
                    int ret = slice_del(z);
                    if (ret < 0) return ret;
                }
    ...
    }
    {
        int v_11 = z->l - z->c;
        {
            ...
            z->ket = z->c;
            if (in_grouping_b(z, g_undouble_c, 98, 116, 0)) { z->lb = v_12; goto lab7; }
            z->bra = z->c;
            {
